...
import (
	...
	"fmt"
	"io/ioutil"
	"misconfig-integrator/app/dbo/entity"
	"misconfig-integrator/app/dto"
...
func (p ProjectServiceImpl) preparingProjectPath(ctx *gin.Context, project entity.Project) projectPathInfo {
	...
	projectPath := fmt.Sprintf("%s%s-misconfig", RepositoryPath, project.Key)
	...
	projectFileName := "scan_repo_file_" + project.Key + "_" + typeVersion + "-misconfig" + ".json"
...
func (p ProjectServiceImpl) mappingResultOutput(filePathResult string, projectId int, projectKey string) []entity.Result {
	...
	for _, result := range data.Results {
		for _, misconfig := range result.Misconfigurations {
			...
			tmpResult.PrimaryUrl = misconfig.PrimaryURL
			tmpResult.Rule = misconfig.ID
			tmpResult.References = strings.Join(misconfig.References, "|")
			tmpResult.Title = misconfig.Title
			tmpResult.Description = misconfig.Description
			tmpResult.Severity = misconfig.Severity
			tmpResult.Path = result.Target
			tmpResult.Type = result.Type
			tmpResult.Message = misconfig.Message
			tmpResult.Resolution = misconfig.Resolution

			causeMetadata := entity.CauseMetadata{
			...
			if len(misconfig.CauseMetadata.Code.Lines) > 0 {
				...
				causeMetadata.StartLine = startLine
			}

			jsonData, _ := json.Marshal(causeMetadata)

			tmpResult.CauseMetadata = string(jsonData)
